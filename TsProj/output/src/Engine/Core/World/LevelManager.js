...
(function (NT) {
    ...
    class LevelManager {
        ...
        static load() {
            ...
            let asset = NT.AssetManager.getAsset("assets/levels/levels.json");
            if (asset !== undefined) {
                LevelManager.processLevelConfigAsset(asset);
            }
            else {
                // Listen for the asset load.
                NT.Message.subscribeCallback(NT.MESSAGE_ASSET_LOADER_ASSET_LOADED + "assets/levels/levels.json", LevelManager.onMessage);
            }
        }
        /**
        ...
        static changeLevel(name) {
            ...
            if (LevelManager._registeredLevels[name] !== undefined) {
                ...
                if (NT.AssetManager.isAssetLoaded(LevelManager._registeredLevels[name])) {
                    let asset = NT.AssetManager.getAsset(LevelManager._registeredLevels[name]);
                    LevelManager.loadLevel(asset);
                }
                else {
                    NT.Message.subscribeCallback(NT.MESSAGE_ASSET_LOADER_ASSET_LOADED + LevelManager._registeredLevels[name], LevelManager.onMessage);
                    NT.AssetManager.loadAsset(LevelManager._registeredLevels[name]);
                }
            }
            else {
        ...
        static onMessage(message) {
            ...
            if (message.code === NT.MESSAGE_ASSET_LOADER_ASSET_LOADED + "assets/levels/levels.json") {
                NT.Message.unsubscribeCallback(NT.MESSAGE_ASSET_LOADER_ASSET_LOADED + "assets/levels/levels.json", LevelManager.onMessage);
                LevelManager.processLevelConfigAsset(message.context);
            }
            else if (message.code.indexOf(NT.MESSAGE_ASSET_LOADER_ASSET_LOADED) !== -1) {

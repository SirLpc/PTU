...
(function (NT) {
    ...
    class MessageBus {
        ...
        static addSubscription(code, handler, callback) {
            ...
            if (matches.length === 0) {
                let node = new NT.MessageSubscriptionNode(code, handler, callback);
        ...
        static post(message) {
            ...
            for (let h of handlers) {
                if (message.priority === NT.MessagePriority.HIGH) {
                    ...
                    MessageBus._normalMessageQueue.push(new NT.MessageQueueNode(message, h.handler, h.callback));
                }
            }
        }
        /**
